...
beta = 1.1 # need to pick a particular beta value which model is trained on.
# other system params, which we want to enforce.
J = 1.
H = 1.
model_size = 16
...
sweep = true
num_runs = 100 # number of times to run MCMC independently, with different initial configurations.
...
# Data will get saved in data/independent/beta folder by default.
...
data_type = "independent" # can also be "raw" to use raw MCMC data for training.
batch_size = 20
...
train_data = NeuralIsing.get_training_data(num_runs, data_type, beta, batch_size, k)
